...
newtype ScancodeNotFound = ScancodeNotFound
  { message :: String
...
get :: Char -> [String]
get char =
  fromMaybe (throw (ScancodeNotFound [char])) (Map.lookup char scancodes)
...
encode1 codePressed normalChar = encode codePressed normalChar Nothing
...
keySeqShift codePressed =
  leftShiftPressed : keySeq codePressed ++ [leftShiftReleased]
